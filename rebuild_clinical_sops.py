...
def split_mega_role_cell(text):
    ...
    for i, word in enumerate(words):
        ...
        # Checks if word is a verb starting a description
    ...
    for idx_in_list, v_idx in enumerate(verb_indices):
        if idx_in_list == 0:
            ...
            # Roles are usually Title Case. We look back from verb.
            ...
            # Update previous entry's responsibility
...
def run_rebuild():
    ...
    print("Wiping existing SOPs for fresh build...")
    ...
    for idx, block in enumerate(iter_block_items(doc)):
        if isinstance(block, Table):
            ...
            if (is_hospital or header_match) and (idx - last_header_idx > 3):
                ...
                current_sop = {
                    'code': '', 'title': '', 'metadata': {}, 'sections': {
                        'purpose': '', 'scope': '', 'definitions': '', 'roles_html': '', 'procedure_html': '',
                        'related': '', 'references': '', 'history_html': '', 'control_html': ''
                    },
                    'current_section': 'purpose',
                    'block_count_in_sop': 0
                }
                last_header_idx = idx
            ...
            if current_sop:
                ...
                # Metadata extraction from first 2 tables
                if b_idx == 1: # Header table
                ...
                elif b_idx == 2: # Date/Author table
                    rows = list(block.rows)
                    if len(rows) >= 2:
                        for i, label in enumerate(rows[0].cells):
                            val = rows[1].cells[i].text.strip() if i < len(rows[1].cells) else ""
                            current_sop['metadata'][label.text.strip().replace(":", "")] = val
                
                # Content processing
                text_up = text.upper()
                if 'ROLES' in text_up and 'RESPONSIBILITY' in text_up:
                    current_sop['current_section'] = 'roles'
                    # Table handling with Mega-Split
                    html = '<div class="table-responsive my-3"><table class="table table-bordered">'
                    html += '<thead class="table-primary text-white"><tr><th>Role</th><th>Responsibility</th></tr></thead><tbody>'
                    rows = list(block.rows)
                    ...
                    for row in rows:
                        ...
                        if c0 and not c1:
                            for r, resp in split_mega_role_cell(c0):
                                html += f'<tr><td class="bg-light fw-bold" style="width:30%">{r}</td><td>{resp}</td></tr>'
                        else:
                            html += f'<tr><td class="bg-light fw-bold" style="width:30%">{c0}</td><td>{c1}</td></tr>'
                    ...
                    for row in block.rows:
                        html += '<tr>' + "".join(f'<td class="p-2">{c.text}</td>' for c in row.cells) + '</tr>'
                    html += '</tbody></table></div>'
            ...
            if text and current_sop:
                ...
                if current_sop['current_section'] == 'procedure':
                    if re.match(r'^\d+\.', text):
                        current_sop['sections']['procedure_html'] += f'<div class="mb-3 d-flex"><span class="fw-bold me-2">{text.split(".",1)[0]}.</span><span>{text.split(".",1)[1].strip() if "." in text else text}</span></div>'
                    else:
                        current_sop['sections']['procedure_html'] += f'<p class="mb-2">{text}</p>'
                else:
                    sec = current_sop['current_section']
                    if sec in current_sop['sections']:
                         current_sop['sections'][sec] += f'<p class="mb-2">{text}</p>'
                    else:
                         current_sop['sections']['procedure_html'] += f'<p class="mb-2">{text}</p>'
    ...
    def assemble_html(s):
        m = s['metadata']
        # Title Page / Control Box
        control_box = f"""
        <div class="sop-control-box p-4 border rounded bg-white shadow-sm mb-5">
            <div class="row align-items-center mb-4">
                <div class="col-8">
                    <h2 class="fw-bold text-primary mb-0">{s['title']}</h2>
                    <p class="text-muted fs-5">{s['code']}</p>
                </div>
                <div class="col-4 text-end">
                    <img src="/static/img/logo.png" style="height: 60px;" onerror="this.style.display='none'">
                    <div class="fw-bold mt-2">Mbuya Dorcas Hospital</div>
                </div>
            </div>
            <div class="row g-0 border rounded overflow-hidden">
                <div class="col-md-4 border-end p-3 bg-light">
                    <div class="small text-muted text-uppercase fw-bold">Effective Date</div>
                    <div class="fs-5">{m.get('Effective Date', 'Jan 2026')}</div>
                </div>
                <div class="col-md-4 border-end p-3 bg-light">
                    <div class="small text-muted text-uppercase fw-bold">Review Date</div>
                    <div class="fs-5">{m.get('Review Date', 'Jan 2027')}</div>
                </div>
                <div class="col-md-4 p-3 bg-light">
                    <div class="small text-muted text-uppercase fw-bold">Version</div>
                    <div class="fs-5">{m.get('Version', '1.0')}</div>
                </div>
            </div>
            <div class="row g-0 border rounded overflow-hidden mt-3">
                <div class="col-md-6 border-end p-3">
                    <div class="small text-muted text-uppercase fw-bold">Prepared By</div>
                    <div>{m.get('Prepared by', 'Quality Office')}</div>
                </div>
                <div class="col-md-6 p-3">
                    <div class="small text-muted text-uppercase fw-bold">Approved By</div>
                    <div>{m.get('Approved by', 'Hospital Management Board')}</div>
                </div>
            </div>
        </div>
        """
        ...
        sections_html = ""
        structure = [
            ("3. Purpose", s['sections']['purpose'] or "To standardize operational clinical procedures."),
            ("4. Scope", s['sections']['scope'] or "All relevant clinical and administrative staff."),
            ("5. Definitions", s['sections']['definitions'] or "N/A"),
            ("6. Roles and Responsibilities", s['sections']['roles_html']),
            ("7. Procedure", s['sections']['procedure_html']),
            ("8. Related Documents", s['sections']['related'] or "N/A"),
            ("9. References", s['sections']['references'] or "National Health Guidelines"),
            ("10. Revision History", s['sections']['history_html'])
        ]
        
        for title, content in structure:
            if content:
                sections_html += f"""
                <div class="sop-section mb-5">
                    <h4 class="fw-bold text-dark border-bottom pb-2 mb-3 mt-4" style="letter-spacing: 0.1rem;">{title}</h4>
                    <div class="sop-section-content ps-2">
                        {content}
                    </div>
                </div>
                """
    ...
    print(f"Committing {len(sops_data)} SOPs...")
    batch = []
    for s in sops_data:
        full_html = assemble_html(s)
        cat = cat_objs.get(CAT_MAP.get(s['prefix'], 'Administrative'), cat_objs['Administrative'])
        batch.append(SOP(
            title=f"{s['code']} - {s['title']}"[:200],
            category=cat,
            content=full_html,
            version=s['metadata'].get('Version', '1.0'),
            status="Published",
            created_by=user
        ))
    
    SOP.objects.bulk_create(batch)
    print("Clinical SOP Rebuild Successful!")
