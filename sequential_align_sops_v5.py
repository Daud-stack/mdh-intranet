...
def run_alignment_v5():
    ...
    print("Alignment V5: Intelligent Extraction...")
    ...
    for idx, block in enumerate(all_blocks):
        ...
        if is_table and "MBUYA DORCAS" in text and ("CODE" in text or "SOP" in text):
            is_split = True
        elif not is_table and re.search(r'MDH-[A-Z]+-\d+', text) and "TITLE" in text:
            is_split = True
    ...
    for i, stream in enumerate(sops_raw):
        ...
        for b in stream:
            ...
            if active_idx == 1:
                ...
                if is_table:
                    cells = [c.text.strip() for r in b.rows for c in r.cells]
                    for c in cells:
                        cup = c.upper()
                        # Better Title discovery
                        if "TITLE:" in cup:
                            title = c.split(":", 1)[1].strip()
                            break
                        if len(c) > 10 and all(x not in cup for x in ["MBUYA", "CODE", "VERSION", "DATE", "CREATED", "APPROVED", "SIGNATURE"]):
                            if not title: title = c.strip()

            for s_idx, _, keywords in SECTIONS_MAP:
                # Look for header paragraphs or table content containing keywords
                if any(kw in (text[:50] if not is_table else text) for kw in keywords) and (len(text) < 150 or is_table):
                    # Be strict about sequence to avoid false positives jumping back
                    if s_idx >= active_idx:
                        active_idx = s_idx
                        break
        ...
        if not title: title = "Standard Operating Procedure"
        title = title.replace("\n", " ").strip().title()
        ...
        batch.append(SOP(
            ...
            category=cat,
            content=f'<div class="sop-standard-markdown">{html}</div>',
            status="Published",
            version="1.0",
            created_by=user
        ))
    ...
    print("V5 Alignment Pass Finished.")
